    ...
    while (1) 
	{
	    ...
	    if(option==1)
		{
			...
			else
			{
				...
				{
					case 1:
						printf("\n    List data by indorder method\n");
						break;
					case 2:
						printf("\n    List data by preorder method\n");
						break;
					case 3:
						printf("\n    List data by postorder method\n");;
						break;
				}
...
//				   	strcpy(temp->address, "   ( Self Pick Up )      ");
	...
	//              	printf("\n    L%2d - %-14s  %2d Kg %-13s %-23s \n\tHas Been Sucessfully Taken\n", temp->ID, temp->name, temp->heavy, temp->number, temp->address);
	    ...
	    else if (option==5)
		{
			...
			{	
				//DEBUG
//					printf("L%d#%s#%d#%d#%s#%s\n", fp_id, fp_nama, fp_berat, fp_harga, fp_no, fp_alamat); fpt, "L%d#%s#%d#%d#%s#%s\n", id, temp->name, temp->heavy, temp->price, temp->number, temp->address); 
				fprintf(fpt, "%d/%02d/%02d#L%d#%s#%d#%d#%s#%s\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, fp_id, fp_nama, fp_berat, fp_harga, fp_no, fp_alamat); 
			}
